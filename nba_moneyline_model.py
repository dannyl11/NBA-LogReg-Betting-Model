...
def predictOutcome(data, predict_data):
    ...
    return win_prob, vegas_probability, vegas_odds #return predicted win prob, book's win prob, and odds for KC calcs
...
def odds_to_decimal(odds):
    return ((int(odds)/100) + 1) if "+" in odds else ((100/abs(int(odds))) + 1)

def kelly(win_prob, raw_odds):
    dec_odds = odds_to_decimal(raw_odds)
    return win_prob - ((1-win_prob) / (dec_odds - 1))

def main():
    data, predict_data = addStats()
    pred, vegas, raw_odds = predictOutcome(data, predict_data)
    print(f'Calculated win probability: {pred}')
    print(f'Book probability: {vegas}')
    print(f'Value over book: {round(pred - vegas, 3)}')
    if pred-vegas > 0:
        kc = kelly(pred, raw_odds)
        print(f'Half Kelly: {kc/2}, Quarter Kelly: {kc/4}')
main()
